...
class GithubRepository(private val api: GithubAPI) {
    ...
    val loadDataState: MutableLiveData<LoadDataState> = MutableLiveData()
    ...
    fun load(apiUrl: String): LiveData<GithubRepo> = RetrofitLiveData<GithubRepo>(api.load(apiUrl), loadDataState)
}